...
class DFPlayer :
    ...
    def __init__( self,
        ...
        self.SetDevice(device if device else DFPlayer.DEVICE_SD)
        if not self.GetState() :
            raise Exception('DFPlayer could not be initialized.')
        self.SetVolume(volume)
        self.SetEqualizer(eq if eq else DFPlayer.EQ_NORMAL)
    ...
    def GetFilesCount(self, device=None) :
        if not device :
            device = self._device
        if device == DFPlayer.DEVICE_U_DISK :
            self._txCmd(0x47)
        elif device == DFPlayer.DEVICE_SD :
            self._txCmd(0x48)
